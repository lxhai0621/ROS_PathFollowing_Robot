...
async def main():
    sbus = await SBUSReceiver.create("/dev/ttyTHS0")
    control_pub = rospy.Publisher("/mycar/remote_control",ControllerMsg,queue_size=1)
    while not rospy.is_shutdown():
        frame = await sbus.get_frame()
        Control_instruction = ControllerMsg()
        Control_instruction.header.stamp = rospy.Time(0)
        Control_instruction.around.data = frame.sbusChannels[1]
        Control_instruction.about.data = frame.sbusChannels[0]
        #rospy.loginfo(Control_instruction)
        control_pub.publish(Control_instruction)
...
if __name__ == '__main__':
    rospy.init_node("remote_controlread")
